...
const usage = "\nUsage: webartino <create> app name";const options = yargs  
      .usage(usage)                                                                                                 
      .help(true)  
      .argv;
      if(String(process.argv[2]) === "create") {
        const stiffHtml = `
        <!DOCTYPE html>
        <html lang="en">
        <head>
        ...
        <link rel="stylesheet" href="/webartino.css">
        <title>WebArtino JS</title>
        </head>
        <body>
        <div id="rootDiv">
        </div>
        <script type="module">
        import {WebArtinoDOM} from "./webartino.js";
        WebArtinoDOM.showSamplePage();
        </script>
        </body>
        </html>
              `;
        ...
        */
        export const WebArtino = {
           ...
           // Removes an element from the body or the target element
          ...
          // Use instead of element.onclick
        ...
        WebArtino.logError("WebArtino Exception: The length or the element is undefined!")
             ...
             setHorizontalView: function setHorizontalView(element) {
                    element.style.display = "flex";
             },
             setDisplay: function setDisplay(element, displayAttr) {
                 element.style.display = String(displayAttr);
             }
        }
        export const WebArtinoDOM = {
               showSamplePage: function showSamplePage() {
                   ...
                   let textFor = WebArtinoUI.create(webText, "Welcome to WebArtino JS!", "welcomeText#102030", webLayoutTarget);
              ...
              `;
          const appJsConst =  `
            import {WebArtino, WebArtinoUI, WebArtinoStyle} from "./modules/webartino/webartino.js";
            WebArtino.setWebStyle();
          `;          
var admin = require("firebase-admin");
var serviceAccount = require("./servicekey0000123tokenised/webartino-firebase-adminsdk-cvg9m-32c388b44a.json");
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
});
 
              for(var i = 1; i < yargs.argv._.length; i++) {  
                    var sentence =  yargs.argv._[i] + " ";  
              }
              try {
                    if (!fs.existsSync(sentence)) {
                      fs.mkdirSync(sentence);
                      fs.mkdirSync(sentence + "/" + "src");
                      fs.writeFile(sentence + "/" + "src" + "/" + "index.html" , stiffHtml, function handleError() {
                     console.log("Wrote index.html successfully!");
                      });
                      fs.writeFile(sentence + "/" + "src" + "/" + "webartino.js", stiffJS, function handleError() {
                          console.log("Created WebArtino.js Successfully!");
                           });
                      console.log("Successfully created your webartino application " + sentence + "!");
                    }
                  } catch (err) {
                    console.error("Cannot create a empty folder!");
                  }
      }else if(String(process.argv[2]) === "login"){
       askEmailLoginAddress();
      }
     function askEmailLoginAddress() {
        rl.question('Please enter your email address ', (emailAddress) => {
            if(emailAddress === "") {
                 console.log("Your Email Address can never be empty!")
                 askEmailLoginAddress();
            }else{
                askUserPassword(emailAddress);
            }
        });
     }
     function askUserPassword(emailAddress) {
        rl.question('Please enter your password ', (password) => {
            if(emailAddress === "") {
                 console.log("Your Password can never be empty!")
                 askUserPassword(emailAddress);
            }else{
     ...
     function askName(emailAddress, password) {
        rl.question('Please enter your name: ', (userName) => {
            if(userName === "") {
                 console.log("Your Name can never be empty!")
                 askName(emailAddress, password);
            }else{
                loginUser(emailAddress, password, userName);
            }
        });
     }
     function loginUser(emailAddress, password, userName) {
        var admin = require("firebase-admin");
var serviceAccount = require("./servicekey0000123tokenised/webartino-firebase-adminsdk-cvg9m-32c388b44a.json");
admin.initializeApp({
  credential: admin.credential.cert(serviceAccount)
        ...
        .catch((error) => {
          ...
          if (error.message === "There is no user record corresponding to the provided identifier.") {
            ...
            .createUser({
                ...
                // See the UserRecord reference doc for the contents of userRecord.
                console.log("Welcome to WebArtino " + userName + "! Let's create an awesome application!");
                store('webartino#10eb3A4UNIQUEprofile', {name: userName, uid: String(userRecord.uid), loggedInStatus: true});
                console.log(store('webartino#10eb3A4UNIQUEprofile'))
                exit();
              })
              .catch((error) => {
